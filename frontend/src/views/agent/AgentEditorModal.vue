          ...
          <div class="settings-container">
            ...
            <div class="settings-content">
              <div class="content-wrapper">
                ...
                <!-- 可调用的智能体（Agent 模式可用） -->
                <div v-show="currentSection === 'subagents'" class="section">
                  <div class="section-header">
                    <h2>可调用的智能体</h2>
                    <p class="section-description">选择此智能体可以调用的子智能体，实现多智能体协作</p>
                  </div>
                  ...
                  <div class="settings-group">
                    ...
                    <div v-if="!isAgentMode" class="setting-row">
                      ...
                      <div class="setting-control">
                        <span class="empty-hint">仅智能体模式可以使用子智能体功能</span>
                      </div>
                    </div>
                    ...
                    <div v-else class="setting-row setting-row-vertical">
                      <div class="setting-info">
                        <label>选择可调用的智能体</label>
                        <p class="desc">此智能体可以将任务委托给以下智能体</p>
                      </div>
                      <div class="setting-control setting-control-full">
                        <div v-if="loadingAgents" class="loading-hint">
                          <t-icon name="loading" :style="{ animation: 'spin 1s linear infinite' }" />
                          加载中...
                        </div>
                        <div v-else-if="availableAgents.length === 0" class="empty-hint">
                          暂无可用的智能体
                        </div>
                        <div v-else class="agents-selector">
                          <div
                            v-for="agent in availableAgents"
                            :key="agent.id"
                            class="agent-select-item"
                            :class="{ 'selected': formData.config.sub_agents?.includes(agent.id) }"
                            @click="toggleSubAgent(agent.id)"
                          >
                            <div class="agent-select-avatar">
                              <AgentAvatar :name="agent.name" size="small" />
                            </div>
                            <div class="agent-select-info">
                              <span class="agent-select-name">{{ agent.name }}</span>
                              <span class="agent-select-desc">{{ agent.description || '无描述' }}</span>
                            </div>
                            <div class="agent-select-check">
                              <t-checkbox :model-value="formData.config.sub_agents?.includes(agent.id)" @change="() => toggleSubAgent(agent.id)" />
                            </div>
                          </div>
                        </div>
                      </div>
                    ...
                    <div v-if="isAgentMode" class="setting-row">
                      ...
                      <div class="setting-control">
                        <span class="count-badge">{{ formData.config.sub_agents?.length || 0 }} 个智能体</span>
                      </div>
                    </div>
                  </div>
                </div>
...
import { createAgent, updateAgent, getPlaceholders, listAgents, type CustomAgent, type PlaceholderDefinition } from '@/api/agent';
...
const navItems = computed(() => {
  ...
  if (isAgentMode.value) {
    ...
    // 可调用的智能体（仅Agent模式）
    items.push({ key: 'subagents', icon: 'usergroup', label: '可调用的智能体' });
  }
  // 有知识库能力时才显示检索策略
...
watch(() => props.visible, async (val) => {
  if (val) {
    ...
    if (props.mode === 'edit' && props.agent) {
      ...
      if (!agentData.config.sub_agents) agentData.config.sub_agents = [];
...
const loadAvailableAgents = async () => {
  ...
  try {
    ...
    // 排除当前正在编辑的智能体
    availableAgents.value = (response.data || []).filter(
      agent => agent.id !== props.agent?.id
    );
  } catch (error) {
...
const toggleSubAgent = (agentId: string) => {
  if (!formData.value.config.sub_agents) {
    formData.value.config.sub_agents = [];
  }
  const index = formData.value.config.sub_agents.indexOf(agentId);
  if (index > -1) {
    formData.value.config.sub_agents.splice(index, 1);
  } else {
    formData.value.config.sub_agents.push(agentId);
  }
};
