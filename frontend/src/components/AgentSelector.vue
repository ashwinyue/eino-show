          ...
          <div v-if="customAgents.length > 0" class="agent-group">
            ...
            >
              ...
              >
                ...
                <span class="agent-option-name">{{ agent.name }}</span>
                <!-- 子智能体标识 -->
                <span v-if="hasSubAgents(agent)" class="subagents-badge">
                  <TIcon name="usergroup" size="10px" />
                  <span>{{ getSubAgentsCount(agent) }}</span>
                </span>
                <svg
                  v-if="currentAgentId === agent.id"
                  width="14"
              ...
              <template #content>
                <div class="agent-tooltip-content">
                  ...
                  <div class="agent-tooltip-capabilities">
                    ...
                    <!-- 子智能体能力标签 -->
                    <div v-if="hasSubAgents(agent)" class="capability-item orchestrator-capability">
                      <TIcon name="usergroup" size="12px" />
                      <span>{{ $t('agent.capabilities.orchestrator', { count: getSubAgentsCount(agent) }) }}</span>
                    </div>
...
// 检查是否有子智能体
const hasSubAgents = (agent: CustomAgent): boolean => {
  return !!(agent.config?.sub_agents && agent.config.sub_agents.length > 0);
};

// 获取子智能体数量
const getSubAgentsCount = (agent: CustomAgent): number => {
