...
show_help() {
    ...
    echo ""
    echo "命令:"
    echo "  start      启动基础设施服务（PostgreSQL + Redis）"
    echo "  stop       停止所有服务"
    echo "  restart    重启所有服务"
    echo "  logs       查看服务日志"
    echo "  status     查看服务状态"
...
start_services() {
    ...
    # 启动服务
    "$DOCKER_COMPOSE_BIN" $DOCKER_COMPOSE_SUBCMD -f docker-compose.yml up -d
    ...
    if [ $? -eq 0 ]; then
        ...
        echo ""
        log_info "接下来的步骤:"
        printf "%b\n" "${YELLOW}  在新终端运行后端:${NC} make dev-app"
        return 0
    else
        log_error "服务启动失败"
        return 1
...
stop_services() {
    ...
    cd "$PROJECT_ROOT"
    "$DOCKER_COMPOSE_BIN" $DOCKER_COMPOSE_SUBCMD -f docker-compose.yml down
